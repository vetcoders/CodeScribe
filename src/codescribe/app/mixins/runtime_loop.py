...
class RuntimeLoopMixin:
    ...
    def _prompt_quit_decision(self) -> str:
        ...
        alert.setMessageText_("Quit CodeScribe?")
        alert.setInformativeText_(
            "CodeScribe can keep the transcription server running in the background "
    ...
    def _stop_backend_server(self) -> bool:
        ...
        pid_file = repo_root / ".pids" / "codescribe-server.pid"
        stopped = False
        ...
        if pid_file.exists():
            try:
                ...
                pid = None
            if pid:
                logger.info("Sending SIGTERM to CodeScribeServer (pid %s)", pid)
                try:
                    ...
                    for _ in range(20):  # wait up to ~2s
                        ...
                        logger.warning("CodeScribeServer pid %s did not exit in time", pid)
                except Exception as exc:
                    logger.warning("Failed to signal CodeScribeServer pid %s: %s", pid, exc)
            try:
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "codescribe.codescribe_server", "stop"],
                cwd=str(repo_root),
            ...
            logger.warning("Unable to stop CodeScribeServer via CLI: %s", exc)
            return False
