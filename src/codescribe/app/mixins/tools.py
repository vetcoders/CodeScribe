...
class ToolsMixin:
    ...
    def _open_chat_demo(self, _sender):
        try:
            ...
            cmd = (
                ...
                f"{shlex.quote(python_exe)} -m codescribe.chatclient "
                f"--base-url {base_arg} "
    ...
    def _open_voice_chat_lab(self, _sender):
        def _worker():
            try:
                host = _backend_host()
                port_file = os.path.join(repo_root(), "logs", "codescribe-server.port")
                candidates: list[int] = []
                ...
                if chosen is None:
                    rumps.AppHelper.callAfter(
                        ...
                        message=(
                            "Could not find the CodeScribe backend. Ensure it is running "
                            "(./CodeScribe start both) and that port 8237 is not taken."
                        ),
                    )
                    return
    ...
    def _export_menu_tree(self, _sender):
        try:
            ...
            mmd_lines = ["```mermaid", "mindmap", "  root((CodeScribe))"]
            ...
            rumps.notification(title="CodeScribe", subtitle="Menu exported", message=md_path)
        except Exception as exc:
            import logging

            logging.getLogger(__name__).error("Failed to export menu: %s", exc, exc_info=True)
            try:
                rumps.alert(title="CodeScribe", message=f"Failed to export menu: {exc}")
            except Exception as exc:
                logger.debug("Suppressed exception", exc_info=exc)
