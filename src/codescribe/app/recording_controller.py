    ...
    async def _finish_recording_locked(self, app: rumps.App) -> None:
        ...
        try:
            ...
            if not client.start_server_if_needed():
                logger.error("Failed to start CodeScribeServer")
                MenuIcon.set(app, MenuIcon.IDLE)
                set_status(app, "Ready")
                self.state = "IDLE"
                self.assistive_mode = False
    ...
    async def _live_transcribe_loop(self, app: rumps.App) -> None:
        try:
            ...
            if not client.check_server_status().get("server"):
                if not client.start_server_if_needed():
                    logger.warning("Live streaming disabled: CodeScribeServer unavailable")
                    return
            iteration = 0
    ...
    async def _start_recording_after_delay(self, app: rumps.App, *, beep_on_start: bool) -> None:
        try:
            ...
            async with self._serial_lock:
                ...
                if not client.start_server_if_needed():
                    logger.error("Hold-start aborted: CodeScribeServer unavailable")
                    return
                if beep_on_start:
                    start_sound()
        ...
        if self.state == "IDLE":
            async with self._serial_lock:
                if self.state != "IDLE":
                    return
                if not client.start_server_if_needed():
                    logger.error("Toggle-start aborted: CodeScribeServer unavailable")
