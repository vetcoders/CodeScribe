# main.py
#
# purpose: entry point for the CodeScribe application. initializes and runs the
...
#
# key components: CodeScribe class (subclass of rumps.app)
...
def acquire_lock():
    ...
    lock_file_path = os.path.join(APP_ROOT, ".codescribe.lock")
...
class CodeScribe(
    ...
    def __init__(self):
        ...
        try:
            ...
            icon_candidates = [
                ...
                os.path.join(repo_root, "src", "codescribe", "assets", "icon.png"),
            ]
            default_icon = next(
    ...
    def _login_plist_path(self) -> str:
        ...
        return os.path.join(home, "Library", "LaunchAgents", "com.codescribe.tray.plist")
    ...
    def _show_toggles_help(self, _sender):
        ...
        try:
            rumps.alert(title="CodeScribe", message=toggles_help_message())
        except Exception as exc:
    ...
    def _install_login_agent(self):
        ...
        app_repo = "/Applications/CodeScribe.app/Contents/Resources/Repo"
        cmd = (
            f"cd '{app_repo}' && ./scripts/quickstart_mac.sh --mode both --daemon --log "
            f"'$HOME/Library/Logs/CodeScribe.app.log'"
        )
        data = {
            "Label": "com.codescribe.tray",
            "ProgramArguments": ["/bin/zsh", "-lc", cmd],
            "RunAtLoad": True,
            "KeepAlive": False,
            "StandardOutPath": os.path.expanduser("~/Library/Logs/CodeScribe.launchd.out.log"),
            "StandardErrorPath": os.path.expanduser("~/Library/Logs/CodeScribe.launchd.err.log"),
        }
...
def _set_process_title() -> None:
    ...
    try:
        ...
        setproctitle.setproctitle("CodeScribeTray")
    except Exception as exc:
...
def run() -> None:
    ...
    if lock_handle is None:
        print("ERROR: Another instance of CodeScribe is already running.")
        print("Stop it first with: ./CodeScribe stop")
        sys.exit(1)
