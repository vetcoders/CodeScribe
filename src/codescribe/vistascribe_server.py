#!/usr/bin/env python3
"""CodeScribeServer – dedicated entrypoint for the local FastAPI backend.
...
* Recording the chosen port for tray clients (`logs/codescribe-server.port`)
...
PID_FILE = PID_DIR / "codescribe-server.pid"
LOG_DIR = REPO_ROOT / "logs"
PORT_FILE = LOG_DIR / "codescribe-server.port"
...
def _ensure_single_instance() -> None:
    ...
    try:
        ...
        raise SystemExit(
            f"CodeScribeServer already running (pid {existing_pid}); use --status/--stop first"
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the CodeScribe backend server")
    parser.add_argument("command", nargs="?", default="start", choices={"start", "status", "stop"})
    parser.add_argument("--bind", default=os.environ.get("CODESCRIBE_BIND", "127.0.0.1"))
    parser.add_argument("--port", type=int, default=None)
    parser.add_argument(
        "--port-fallbacks",
        default=os.environ.get("CODESCRIBE_PORT_FALLBACKS", ",".join(map(str, DEFAULT_PORTS))),
    ...
    if args.command == "status":
        ...
        print(f"CodeScribeServer: {status} (port {port_info})")
        return 0
    ...
    if args.command == "stop":
        if PID_FILE.exists():
            try:
                ...
                print(f"Sent SIGTERM to CodeScribeServer (pid {pid})")
            except Exception as exc:
                print(f"Could not stop server cleanly: {exc}", file=sys.stderr)
        else:
            print("CodeScribeServer is not running")
        return 0
    ...
    if _set_proc_title_fn is not None:
        try:
            _set_proc_title_fn("CodeScribeServer")
        except Exception as exc:
            logger.debug("Suppressed exception", exc_info=exc)

    logging.getLogger(__name__).info("Starting CodeScribeServer on %s:%s", bind_host, chosen_port)
