"""Lightweight HTTP client for CodeScribe server operations.

This module provides HTTP-based transcription and formatting to keep the tray process light.
All heavy ML operations are delegated to the CodeScribeServer process.
"""
...
try:
    __version__ = metadata.version("codescribe")
except Exception:  # pragma: no cover - local editable installs may miss metadata
    __version__ = "dev"

DEFAULT_PORTS: tuple[int, ...] = (8237, 7237, 6237, 5237)
USER_AGENT = f"CodeScribeClient/{__version__}"
ALLOW_REMOTE_ENV = os.environ.get("ALLOW_REMOTE_CODESCRIBE") or os.environ.get("ALLOW_REMOTE")
PORT_FILE = repo_root() / "logs" / "codescribe-server.port"


def _backend_host() -> str:
    host = os.environ.get("CODESCRIBE_HOST", "127.0.0.1").strip()
...
def _explicit_url_candidate() -> str | None:
    candidate = _normalize_url(os.environ.get("CODESCRIBE_SERVER_URL"))
    if not candidate:
        return None
...
async def transcribe_http(
    ...
    if not server_url:
        logging.error("CodeScribeServer not found. Please start the server.")
        return None
...
async def format_text_http(
    ...
    server_url = await resolve_server_url()
    if not server_url:
        logging.warning("CodeScribeServer not found. Returning baseline text.")
        return baseline_text
...
def check_server_status() -> dict[str, bool]:
    """Check if CodeScribeServer is running and what services are available.
...
def _spawn_backend_process() -> bool:
    """Attempt to spawn the dedicated CodeScribeServer entrypoint."""

    python_exe = sys.executable or "python3"
    logs_dir = repo_root() / "logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    log_path = logs_dir / "codescribe-server.autostart.log"
    env = os.environ.copy()
    ...
    try:
        subprocess.Popen(
            [python_exe, "-m", "codescribe.codescribe_server", "start"],
            cwd=str(repo_root()),
            stdout=log_file,
            stderr=log_file,
            env=env,
...
def _spawn_backend_via_launcher() -> bool:
    script_path = repo_root() / "CodeScribe"
...
def start_server_if_needed() -> bool:
    """Ensure CodeScribeServer is running; auto-start if necessary."""
    ...
    for _ in range(20):  # Wait up to ~10 seconds
        time.sleep(0.5)
        if check_server_status()["server"]:
            logging.info("CodeScribeServer started successfully")
            return True

    logging.error("Timed out waiting for CodeScribeServer to start")
    return False
