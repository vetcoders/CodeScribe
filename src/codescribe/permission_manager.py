...
class PermissionManager:
    ...
    def _format_permission_summary(self, info: dict[str, Any]) -> tuple[str, str]:
        ...
        details.append(
            ...
            if ax
            else "Accessibility: missing (enable CodeScribe in Accessibility)."
        )
        details.append(
            "Hotkeys/Input Monitoring: ready"
            if tap_ok
            else ("Hotkeys/Input Monitoring: missing (enable CodeScribe in Input Monitoring).")
        )
        details.append(
        ...
        if granted:
            ...
            rumps.alert(
                ...
                message=(
                    ...
                    "Microphone and enable CodeScribe."
                ),
                ok="OK",
            )
        self._schedule_permission_probe()
    ...
    def _open_accessibility(self, _sender) -> None:
        self._open_privacy_anchor(
            ...
            ("Open System Settings → Privacy & Security → Accessibility and enable CodeScribe."),
        )

    def _open_microphone_settings(self, _sender) -> None:
        self._open_privacy_anchor(
            "Privacy_Microphone",
            ("Open System Settings → Privacy & Security → Microphone and enable CodeScribe."),
        )
