...
pub type InferFn =
    Arc<dyn Fn(Vec<f32>, u32, Option<String>) -> Result<RawTranscript> + Send + Sync>;
pub type CommitPrefilterFn = Arc<dyn Fn(&[f32], u32) -> Vec<f32> + Send + Sync>;
...
impl SttTaskHandle {
    ...
    pub async fn recv(&mut self) -> Result<RawTranscript> {
...
impl SttScheduler {
    ...
    pub fn with_infer_fn(infer_fn: InferFn) -> Self {
        Self::with_runtime_fns(infer_fn, Arc::new(default_commit_prefilter))
    }

    pub fn with_infer_and_commit_prefilter(
        infer_fn: InferFn,
